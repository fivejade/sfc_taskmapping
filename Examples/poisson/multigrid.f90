module multigrid
    ...
    subroutine multigrid_create(sdm, nlevel, ncycle, aggr_method, aggr_level)
        ...
        integer(kind=4), intent(in)     :: aggr_level
...
! #endif
    ...
    subroutine multigrid_solve_vcycle(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        ...
        ! timer info.
        ! 10 : multigrid V-cycle solver
        call timer_start(10)
        select case(aggregation_type)
        case(0)
            call multigrid_no_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        case(1)
            call multigrid_single_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        case(2)
            call multigrid_adaptive_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        case default
            if(myrank.eq.0) print '(a,i2)', '[Error] Aggregation method should be 0, 1, or 2.', aggregation_type
            call MPI_Finalize(ierr)
            stop
    ...
    subroutine multigrid_no_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
    ...
    subroutine multigrid_single_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        ...
        do cyc = 1, n_vcycles
            ...
            if(myrank.eq.0) then
                do l = lv_aggregation, n_levels-1
                    ...
                    call multigrid_restriction(mg_sdm(l+1)%b, mg_sdm(l)%r, mg_sdm(l+1), mg_sdm(l),l)
                    call timer_stamp(13)
                    print '(a,i2,a,i2)', '[MG] Restricton from level ',l,' to level ',l+1
                enddo
    ...
    subroutine multigrid_adaptive_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        ...
        do cyc = 1, n_vcycles
                ...
                if(comm_med_ptr%myrank.eq.0) then
                    ...
                    if(comm_max_ptr%myrank.eq.0) then
                        ...
                        do l = n_levels-1, max0(1, lv_aggr_max_ptr), -1
                            ...
                            mg_sdm(l)%x = mg_sdm(l)%x + mg_sdm(l)%r
                            call timer_stamp(14)
                            print '(a,i2,a,i2)', '[MG] Prolongation from level ',l+1,' to level ',l
                            call rbgs_iterator_poisson_matrix(mg_sdm(l)%x, mg_a_poisson(l), mg_sdm(l)%b, mg_sdm(l), maxiteration, omega_sor, mg_sdm(l)%is_aggregated)
                            call timer_stamp(12)
                        enddo
                    endif
                    ...
                    do l = lv_aggr_max_ptr-1, max0(1, lv_aggr_med_ptr), -1
                        ...
                        mg_sdm(l)%x = mg_sdm(l)%x + mg_sdm(l)%r
                        call timer_stamp(14)
                        if(myrank.eq.0) print '(a,i2,a,i2)', '[MG] Prolongation from level ',l+1,' to level ',l
                        call rbgs_iterator_poisson_matrix(mg_sdm(l)%x, mg_a_poisson(l), mg_sdm(l)%b, mg_sdm(l), maxiteration, omega_sor, mg_sdm(l)%is_aggregated)
            ...
            sol = sol + rsd
            call timer_stamp(14)
            if(myrank.eq.0) print '(a,i2,a,i2)', '[MG] Prolongation from level ',1,' to level ',0

program main
    ...
    integer(kind=4) :: i, j, k, maxiteration, number_of_vcycles, number_of_levels, aggregation_method, aggregation_level
...
!    -(2*a*exp(a*(x^2 - 1)))/(1-exp(-a)) - (4*a^2*x^2*exp(a*(x^2 - 1)))/(1-exp(-a))
!    -(2*a + 4*a^2*x^2)*exp(a*(x^2 - 1))/(1-exp(-a))
!    -(2*a + 4*a^2*x^2)*exp(a*(x^2 - 1))/(1-exp(-a))
    ...
    call multigrid_create(s_domain, number_of_levels, number_of_vcycles, aggregation_method, aggregation_level)
    call multigrid_solve_vcycle(s_domain%x, a_poisson, s_domain%b, s_domain, maxiteration, tolerance, omega_sor)
    ...
    if(myrank.eq.0) then
        do i = 1, 36
            print '(a,i3, f15.6 f15.6 f15.6 f15.6)','[Poisson] Timer info. : ',i,t_array(i), t_array_max(i), t_array_min(i), t_array_sum(i)/nprocs
        enddo
    endif
